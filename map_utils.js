...
var create_layer_tms, tmsoverlay;
var mapBounds = new OpenLayers.Bounds(97.3436965942383,5.61273717880249,105.637031555176,20.4649257659912);
var mapMinZoom = 14;
var mapMaxZoom = 20;
...
var uav_edit_frm;
var popup_uav_edit, uav_edit_frm;
...
function getURL(bounds) {
  ...
  var path = "/uav-png/" + z + "/" + x + "/" + y + "." + this.type; 
  var url = this.url;
  if (OpenLayers.Util.isArray(url)) {
    url = this.selectUrl(path, url);
  }

  if (mapBounds.intersectsBounds(bounds.transform(merc,gcs)) && (z >= mapMinZoom) && (z <= mapMaxZoom)) {
    return url + path;
  } else {
    return emptyTileURL;
  }
...
create_layer_tms = function() {
  if (tmsoverlay) {
    map.removeLayer(tmsoverlay);
    tmsoverlay = null;
  }
...
var search_uav = function() {
  var q = Ext.getCmp('id_uavtextfield').value;
  proxy.url = "rb/search-flightplan.rb?query=" + q;
  uavstore.load();
};
...
var searchuavfrm = Ext.create("Ext.form.Panel",{
  id: 'id_searchuavfrm'
  ,labelAlign: 'left'
  ,align: 'center'
  ,frame: true
  ...
  ,items: [{
    ...
    ,buttons: [{
      ...
      text: 'Clear'
      ,handler: function(){
        searchuavfrm.getForm().reset();
        // Clear uavgrid data
        uavgrid.store.clearData();
        uavgrid.view.refresh();

        // hide KML and TMS layers
        kml.setVisibility(false);
        tmsoverlay.setVisibility(false);
      }
    }]
  }]
});

function uavspecialKey(field, e) {
  if ( e.getKey() == e.RETURN || e.getKey() == e.ENTER ) {
    search_uav();
  }
}
...
var proxy = new Ext.data.HttpProxy({
  method: 'POST'
  ,url:'rb/search-flightplan.rb'
});

// create uavstore
var uavstore = new Ext.data.Store({
  proxy:proxy
  ,fields: [
    { name: 'id'}
    ,{ name: 'flightdate' }
...
// create the grid
var uavgrid = new Ext.grid.GridPanel({
  store: uavstore
  ,columns: [
    {header: "Flight Date", width: 60, dataIndex: 'flightdate', sortable: true}
    ,{header: "Flight No", width: 60, dataIndex: 'flightno', sortable: true}
    ,{header: "Description", width: 60, dataIndex: 'description', sortable: true}
    ,{
       xtype: 'actioncolumn'
       ,header: 'Action'
       ,width: 60
       ,items: [{
         icon: 'img/eye.png'
         ,tooltip: 'Show this Flight Plan'
         ,handler: function(grid, rowIndex, colIndex) {
           var rec = grid.store.data.items[rowIndex];
           var id = rec.data.id;
           var kmlname = rec.data.kmlname;
           var center = rec.data.center;

           // Create 2 layers
           create_layer_kml(kmlname);
           create_layer_tms();

           // and setCenter for the above flight plan 
           var poi = new OpenLayers.LonLat.fromString(center);
           poi.transform(gcs,merc);
           map.setCenter(poi, 14);
         }
       },{
         icon: 'img/edit.png'
         ,tooltip: 'Edit this Flight Plan'
         ,handler: function(grid, rowIndex, colIndex) {
           var rec = grid.store.data.items[rowIndex];
           var id = rec.data.id;
           var fdate = rec.data.flightdate;
           var fno = rec.data.flightno;
           var descr = rec.data.description;
           var kmlname = rec.data.kmlname;
           var center = rec.data.center;
           show_uav_edit_form(id,fdate,fno,descr);
         }
       }]  
     }
  ]
...
var uav_panel = Ext.create("Ext.Panel", {
  title: "ค้นหา Flight Plan (UAV)"
  ,listeners: {
    expand: function() {
      Ext.getCmp('id_uavtextfield').focus();
    }
  }
  ,items: [ searchuavfrm, uavgrid ]
});
...
var loadtiff = Ext.create("Ext.form.Panel",{
  id: 'id_load_tiff'
  ,title: 'TIFF'
  ,width: 500
  ,frame: true
  ...
  ,buttons: [{
    ...
    ,handler: function() {
      ...
      if (form.isValid()) {
        form.submit({
          url: 'rb/file-upload-tiff.rb'
          ,waitMsg: 'Uploading TIFF...'
          //,success: function(response, opts) { NOT WORKING ?!?!?
          ,success: function(fp, o) {
            var data = Ext.decode(o.response.responseText);  
            ////TO BE FIXED var kmlname = data.kmlname;
            ////TO BE FIXED create_layer_kml(kmlname);                       
          }
        })
      }
    }
...
var loaduav = Ext.create("Ext.form.Panel",{
  id: 'id_load_uav'
  ...
  ,buttons: [{
    ...
    ,handler: function() {
      ...
      if (form.isValid()) {
        form.submit({
          url: 'rb/file-upload-uav.rb'
          ,waitMsg: 'Uploading Flight Plan...'
          ,success: function(fp, o) {
            var data = Ext.decode(o.response.responseText);  
            var kmlname = data.kmlname;
            var center = data.center;
            center = new OpenLayers.LonLat.fromString(center);
            create_layer_kml(kmlname);
            create_layer_tms();
            map.setCenter(center.transform(gcs,merc), 14);
          }
        })
      }
    }
...
uav_edit_frm = Ext.create('Ext.form.Panel', {
  title: ''
  ,id: 'id_uav_edit'
  ,url: 'rb/update-uav.rb'
  ...
  buttons: [{
    ...
    text: 'Delete'
    ,handler: function() {
      var id = Ext.getCmp('uav_id').value;
      var question = "ต้องการลบ ID: " + id + " ออกจากฐานข้อมูล ใช่หรือไม่ ?";
      Ext.Msg.confirm(
        'Confirm'
        ,question
        ,function (btn){
          if(btn=='yes') {
            // STEP1: hide popup EDIT UAV
            popup_uav_edit.hide();

            // STEP2: delete from database
            Ext.Ajax.request({
              url: 'rb/delete-uav.rb'
              ,params: { id: id }
              ,success: function(resp,opt) {
                info('Result', 'ลบ ID: ' + id + ' ออกจากฐานข้อมูลเรียบร้อยแล้ว');
              }
              ,failure: function(resp, opt) {
                Ext.Msg.alert('Warning', 'เกิดข้อผิดพลาดไม่สามารถลบรายการที่ต้องการได้');
              }
            ...
            // STEP3: reload uavstore
            uavstore.load();

            // STEP4: refresh grid data
            uavgrid.store.clearData();
            uavgrid.view.refresh();
...
function show_uav_edit_form(id,fdate,fno,descr) {
  if (!popup_uav_edit) {
    popup_uav_edit = Ext.create("Ext.window.Window", {
      title: "EDIT UAV"
      ,width: 250
      ,height: 175
      ,layout: 'fit'
  ...
  // populate date in uav_edit_frm
  Ext.getCmp('uav_id').setValue(id);
  Ext.getCmp('uav_fdate').setValue(fdate);
  Ext.getCmp('uav_fno').setValue(fno);
  Ext.getCmp('uav_descr').setValue(descr);

  // and show window with for ready to edit
  popup_uav_edit.show();
}
